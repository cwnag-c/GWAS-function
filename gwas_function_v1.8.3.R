...
GWAS_map_chrpos<-function(f1=NULL,MAP_dir,gwas_dir,pre_col_name,post_col_name){
  ...
  if(!exists("MAP_file")){
    ...
    MAP_file %<>% 
      ...
      rename(SNP=rsmid,CHR=chr,POS=pos)
    }
...
GWAS_map_snp<-function(f1=NULL,MAP_dir,gwas_dir,pre_col_name,post_col_name,
  ...
  if(character_chr){
    ...
    f1 %<>%mutate(CHR=as.integer(CHR))###默认转化为int
  }
...
GWAS_liftover_hg38to37<-function(liftover_dir,gwas_dir,pre_col_name,post_col_name,
  ...
  if(character_chr){
    ...
    gwas_f1 %<>%mutate(CHR=as.integer(CHR))###默认转化为int
  }
  gwas_f1 %<>% 
    mutate(A1=toupper(A1)) %>%
    mutate(A2=toupper(A2)) %>% 
    filter(A1 %in% c("A","T","G","C")) %>% filter(A2 %in% c("A","T","G","C")) 
...
GWAS_clean_noEAF<-function(f1=NULL,gwas_dir,Samplesize,map_dir,ref_dir,
  ...
  if(is_character_chr){
    ...
    f1 %<>% dplyr::mutate(CHR=as.integer(CHR))###默认转化为int
  }
      ...
      if(nrow(f1_exposure)!=0){ #如果没有显著的点，就跳过#
        ...
        res<-lapply(1:nrow(outcome_list), function(c){
          ...
          f2_outcome<-format_data(outcome,type = "outcome",snps = NULL,header = TRUE,
                                  ...
                                  chr_col = "CHR",pos_col = "POS",samplesize_col="N",
                                  min_pval = 1e-500,log_pval = FALSE)
          dat<-harmonise_data(exposure_dat = f1_exposure_clump,outcome_dat =f2_outcome,action=Action)
          #------------MR steiger---------#
          dat<-steiger_filtering(dat)
          #---------------F值计算--------------#
          dat %<>%
...
GWAS_twosampleMR<-function(pattern,out_path,mr_name,is_presso=F,is_steiger=F){
  library(TwoSampleMR)
  cat("*******************************************************************\n")
  cat("* CWAS \n")
  cat("* Version 1.6\n")
  cat("* Update Date:2024.07.11\n")
  cat("* (C) 2024 Chao Wang \n")
  cat("* Nanjing Medical University\n")
  cat("*******************************************************************\n")
  ...
  if(pattern==1){
    ...
    MR_res<-lapply(list_name, function(i){
      ...
      return(list(mr_res=restBindsub,leave_one_out=leaveone_out,presso_res=mr_presso))
    })
    names(MR_res)<-list_name
    
    #--------------提出和输出结果--------------------#
    mr_res_list <- lapply(MR_res, function(x) x$mr_res)###提取twosample MR结果
    mr_res_list<-bind_rows(mr_res_list)
    write.csv(mr_res_list,file = paste0(out_path,mr_name,".mr_res.csv"))
    
    leaveone_out_list <- lapply(MR_res, function(x) x$leave_one_out) ###提取leave on out结果
    ...
    if(is_presso){
      mr_presso<-lapply(list_name, function(i){
        tmp<-MR_res[[i]][["presso_res"]][[1]][["Main MR results"]]
        if(is.null(tmp)==F){
          tmp$e2o<-i
          return(tmp)
        }
      ...
      write.csv(mr_presso,file = paste0(out_path,mr_name,"mr_presso.csv"))
    }#如果为Ture，则提出presso结果；如果false，本身就是null，不提。
    
  }

}
GWAS_heterogeneity_lasso_ivw<-function(out_path,mr_name,mr_pair_list){
  ...
  load(rdata_dir)
  IVs<-ivs$ivs
  mr_het_res<-lapply(mr_pair_list, function(i){
    #####mr_lasso提出无效snp
    dat=IVs[[i]]
    dat %<>% dplyr::filter(ambiguous=="FALSE")
    mr_data<-mr_input(bx=dat$beta.exposure,
                      bxse=dat$se.exposure,
                      by=dat$beta.outcome,
                      byse = dat$se.outcome,
                      exposure = i,outcome = i,
                      snps = dat$SNP,
                      effect_allele = dat$effect_allele.exposure,
                      other_allele = dat$other_allele.exposure,
                      eaf = NA)
    lasso_res<-mr_lasso(mr_data)
    lasso_valid_snps<-lasso_res@ValidSNPs
    dat_lasso<-dat[dat$SNP %in% lasso_valid_snps,]
    
    ######重跑ivw
    res<-mr(dat_lasso,method_list=c("mr_ivw"))
    het<-mr_heterogeneity(dat_lasso)
    ple<-mr_pleiotropy_test(dat_lasso)
    restBindsub<-bind_rows(res,het,ple)
    return(mr_res=restBindsub)
  })
  names(mr_het_res)<-mr_pair_list
  #####输出
  mr_het_res<-bind_rows(mr_het_res)
  write.csv(mr_het_res,file = paste0(out_path,mr_name,".mr_het_lasso_ivw.csv"))
}
#----------------4.2 MR:GSMR2----------------
#----------------4.3 MR:CAUSE---------------
#----------------4.4 MR:MVMR---------------
